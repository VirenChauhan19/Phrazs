...
export default function MobileMap({ items, activeId, setActiveId }) {
  ...
  // When a pin is tapped (activeId changes), centre that card in the carousel.
  useEffect(() => {
    if (!activeId) return;
  ...
  // As the carousel settles on a card, select it (which pans the map).
  const onScroll = () => {
  ...
  return (
    <div className="m-map">
      <div className="m-map__canvas">
        <MapView listings={points} activeId={activeId} onSelect={setActiveId} showPopups={false} />
      </div>
